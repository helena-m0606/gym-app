...
const maxFecha = new Date(new Date().setFullYear(new Date().getFullYear() - 15)).toISOString().split('T')[0];

type Sucursal = { id: number; nombre: string };
type Miembro = {
    id: number; nombre: string; email: string; telefono: string | null;
    fecha_nacimiento: string | null; genero: string | null; estado: boolean; sucursal: Sucursal;
};
...
export default function MiembrosIndex({ miembros, sucursales }: { miembros: Miembro[]; sucursales: Sucursal[] }) {
    const [showForm, setShowForm] = useState(false);

    const { data, setData, post, processing, errors, reset } = useForm({
        sucursal_id: '', nombre: '', email: '', telefono: '', fecha_nacimiento: '', genero: '',
    });
    ...
    const editForm = useForm({
        sucursal_id: '', nombre: '', email: '', telefono: '',
        fecha_nacimiento: '', genero: '', estado: true as boolean,
    });

    function openEdit(m: Miembro) {
        editForm.setData({
            sucursal_id: String(m.sucursal.id), nombre: m.nombre, email: m.email,
            telefono: m.telefono ?? '', fecha_nacimiento: m.fecha_nacimiento ?? '',
            genero: m.genero ?? '', estado: m.estado,
        });
        setEditTarget(m);
    }
                ...
                {!showForm ? (
                    ...
                    <form onSubmit={submit} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                        <div className="mb-5 flex items-center justify-between">
                            <h3 className="text-lg font-semibold">Registrar nuevo miembro</h3>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <input type="text" placeholder="Teléfono" value={data.telefono} maxLength={10}
                                    onChange={(e) => setData('telefono', e.target.value.replace(/\D/g, ''))} className={inputCls} />
                            </div>
                            <div>
                                <input type="date" value={data.fecha_nacimiento} max={maxFecha}
                                    onChange={(e) => setData('fecha_nacimiento', e.target.value)} className={inputCls} />
                            </div>
                            <div>
                                <select value={data.genero} onChange={(e) => setData('genero', e.target.value)} className={inputCls}>
                ...
                <form onSubmit={submitEdit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2">
                            ...
                            <input type="email" value={editForm.data.email} onChange={(e) => editForm.setData('email', e.target.value)} className={inputCls} />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-medium text-gray-500">Teléfono</label>
                            <input type="text" value={editForm.data.telefono} maxLength={10}
                                onChange={(e) => editForm.setData('telefono', e.target.value.replace(/\D/g, ''))} className={inputCls} />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-medium text-gray-500">Fecha de nacimiento</label>
                            <input type="date" value={editForm.data.fecha_nacimiento} max={maxFecha}
                                onChange={(e) => editForm.setData('fecha_nacimiento', e.target.value)} className={inputCls} />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-medium text-gray-500">Género</label>
                            <select value={editForm.data.genero} onChange={(e) => editForm.setData('genero', e.target.value)} className={inputCls}>
                                <option value="">Seleccionar género</option>
                                <option value="Masculino">Masculino</option>
                                <option value="Femenino">Femenino</option>
                                <option value="Otro">Otro</option>
                        ...
                        <div>
                            ...
                            <select value={editForm.data.estado ? '1' : '0'} onChange={(e) => editForm.setData('estado', e.target.value === '1')} className={inputCls}>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button type="button" onClick={() => setEditTarget(null)}
                            className="rounded-xl border border-gray-200 px-5 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50">Cancelar</button>
