...
function Modal({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">{children}</div>
        </div>
    );
}
...
export default function Index({ rutinas, catalogoEjercicios, entrenadores }: { rutinas: Rutina[]; catalogoEjercicios: ItemCatalogo[]; entrenadores: ItemCatalogo[] }) {
    ...
    function openEdit(r: Rutina) {
        const ejerciciosMapeados = r.ejercicios ? r.ejercicios.map(ej => ({
    ...
    function agregarFilaEjercicioEdit() {
        editForm.setData('ejercicios', [
            ...editForm.data.ejercicios,
            { ejercicio_id: '', series: '4', repeticiones: '12' }
        ]);
    }

    function removerFilaEjercicioEdit(index: number) {
        editForm.setData('ejercicios', editForm.data.ejercicios.filter((_, i) => i !== index));
    }

    function manejarCambioFilaEdit(index: number, campo: keyof EjercicioForm, valor: string) {
        const actualizados = editForm.data.ejercicios.map((fila, i) => i === index ? { ...fila, [campo]: valor } : fila);
        editForm.setData('ejercicios', actualizados);
    }
    ...
    return (
        ...
        >
                    ...
                    <form onSubmit={submit} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
                        ...
                        <div className="border-t border-gray-100 pt-4 space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-semibold text-gray-700">Ejercicios incluidos en la rutina</span>
                                <button
                                    type="button"
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div className="border-b border-gray-200 p-5 font-semibold">Rutinas Registradas en el Sistema</div>

                <div className="block md:hidden divide-y divide-gray-100">
                    {rutinas.length === 0 ? (
                        <div className="p-5 text-center text-sm text-gray-400">No hay rutinas registradas.</div>
                    ) : (
                        rutinas.map((r) => (
                ...
                <div className="hidden md:block overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 text-gray-500">
                            <tr>
                                <th className="p-5 w-1/4">Nombre de la Rutina</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {rutinas.length === 0 ? (
                                <tr><td colSpan={4} className="p-5 text-center text-gray-400">No hay rutinas.</td></tr>
                            ) : (
                                rutinas.map((r) => (
            ...
            {/* ── Modal Editar Completo (Nombre, Autor y Ejercicios) ── */}
            <Modal open={!!editTarget} onClose={() => setEditTarget(null)}>
                <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                    <h3 className="text-lg font-semibold text-gray-800">Modificar Rutina</h3>
                    <button type="button" onClick={() => setEditTarget(null)} className="text-xs font-bold text-gray-400 hover:text-gray-600 transition">Cerrar</button>
                </div>
                <form onSubmit={submitEdit} className="space-y-4">
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                        <div className="sm:col-span-2">
                            <label className="mb-1 block text-xs font-medium text-gray-400">Nombre de la Rutina</label>
                            <input type="text" value={editForm.data.nombre} onChange={(e) => editForm.setData('nombre', e.target.value)} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-orange-400" required />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-medium text-gray-400">Entrenador</label>
                            <select value={editForm.data.entrenador_id} onChange={(e) => editForm.setData('entrenador_id', e.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:border-orange-400 text-gray-700" required>
                                {entrenadores.map((ent) => (
                                    <option key={ent.id} value={ent.id}>{ent.nombre}</option>
                                ))}
                            </select>
                    ...
                    <div className="border-t border-gray-100 pt-3 space-y-2">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Ejercicios</span>
                            <button type="button" onClick={agregarFilaEjercicioEdit} className="text-[11px] font-bold text-orange-500 bg-orange-50 px-2 py-1 rounded-md transition">+ Añadir</button>
                        </div>
                            ...
                            {editForm.data.ejercicios.map((fila, index) => (
                                <div key={index} className="grid grid-cols-12 gap-2 bg-gray-50 p-2 rounded-xl border border-gray-100 items-center">
                                    <div className="col-span-6">
                                        <select value={fila.ejercicio_id} onChange={(e) => manejarCambioFilaEdit(index, 'ejercicio_id', e.target.value)} className="w-full rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-xs outline-none focus:border-orange-400">
                                    ...
                                    <div className="col-span-2">
                                        <input type="number" value={fila.series} onChange={(e) => manejarCambioFilaEdit(index, 'series', e.target.value)} className="w-full text-center rounded-lg border border-gray-200 bg-white px-1 py-1.5 text-xs outline-none focus:border-orange-400" placeholder="S" title="Series" />
                                    </div>
                                    <div className="col-span-3">
                                        <input type="number" value={fila.repeticiones} onChange={(e) => manejarCambioFilaEdit(index, 'repeticiones', e.target.value)} className="w-full text-center rounded-lg border border-gray-200 bg-white px-1 py-1.5 text-xs outline-none focus:border-orange-400" placeholder="Reps" title="Repeticiones" />
                                    </div>
                                    <div className="col-span-1 flex justify-center">
                                        <button type="button" onClick={() => removerFilaEjercicioEdit(index)} className="text-gray-400 hover:text-red-500 transition"><IconTrash /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        ...
                        <button type="submit" disabled={editForm.processing} className="rounded-xl bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-orange-600">Guardar Cambios</button>
                    </div>
                </form>
            </Modal>
